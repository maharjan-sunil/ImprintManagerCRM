...
{
    public class TenantRegistrationCommandHandler : IRequestHandler<TenantRegistrationCommand, Result<long>>
    {
        ...
        public TenantRegistrationCommandHandler(ISharedServiceDbContext sharedServiceDbContext, IEmailClientService emailService, UserManager<User> userManager, RoleManager<Role> roleManager, IConfiguration config)
        {
        ...
        public async Task<Result<long>> Handle(TenantRegistrationCommand command, CancellationToken cancellationToken)
        {
            try
            {
